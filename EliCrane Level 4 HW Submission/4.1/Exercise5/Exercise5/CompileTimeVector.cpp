...
//Get const Array
template <typename T, int N>
const std::array<T, N>& Vector<T, N>::GetArr() const{
	return arr;
}
...
//Addition
template <typename T, int N>
const Vector<T, N> Vector<T, N>::operator+(const Vector<T, N>& v2) const {
...
//Unary Subtraction
template <typename T, int N>
const Vector<T, N> Vector<T, N>::operator-(const T& t) const {
